    ...
    return {
        ...
        addXp(amount) {
            ...
            if (window.achievementSystem) {
                const unlocked = window.achievementSystem.checkAchievements(this);
                unlocked.forEach(achievement => {
                    this.showNotification('実績解除！', `${achievement.title} (+${achievement.xp} XP)`);
                    // Recursively add XP from achievements without infinite loop risk (achivement XP shouldn't trigger more achievements usually, but good to be aware)
        ...
        levelUp() {
            ...
            try {
                ...
                audio.play().catch(e => console.log('Audio play failed (user interaction or missing file):', e));
            } catch (e) {
        ...
        setupDailyQuest() {
            ...
            let savedQuest = null;
            try {
                savedQuest = JSON.parse(localStorage.getItem('dailyQuest'));
            } catch (e) {
                console.error('Failed to parse dailyQuest:', e);
        ...
        loadFromLocalStorage() {
            let saveData = null;
            try {
                saveData = JSON.parse(localStorage.getItem('focusMountainSave'));
            } catch (e) {
                console.error('Failed to load save data:', e);
                // Optionally reset save data here or just start fresh
                localStorage.removeItem('focusMountainSave');
            }
